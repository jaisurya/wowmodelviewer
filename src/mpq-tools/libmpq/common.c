...
int libmpq_file_read_block(mpq_archive *mpq_a, mpq_file *mpq_f, unsigned int blockpos, char *buffer, unsigned int blockbytes) {
	...
	if ((mpq_f->mpq_b->flags & LIBMPQ_FILE_COMPRESSED) && 
		...
		!(mpq_f->mpq_b->flags & LIBMPQ_FILE_SINGLE_UNIT)) {
		unsigned int nread;
	...
	if (mpq_f->mpq_b->flags & LIBMPQ_FILE_COMPRESSED && !(mpq_f->mpq_b->flags & LIBMPQ_FILE_SINGLE_UNIT)) {
...
/* single file ? */
int libmpq_file_read_single_file(mpq_archive *mpq_a, mpq_file *mpq_f, unsigned int filepos, char *buffer, unsigned int toread) {
	unsigned int bytesread = 0, bytestoread = toread;			/* Number of bytes read from the file */
	int outlength = mpq_f->mpq_b->fsize;
	char *tempbuf;

	/* Set file pointer, if necessary. */
	if (mpq_a->filepos != mpq_f->mpq_b->filepos) {
		mpq_a->filepos = lseek(mpq_a->fd, mpq_f->mpq_b->filepos, SEEK_SET);
	}
	...
	while(bytestoread > 0) {
		bytesread = read(mpq_a->fd, &tempbuf[bytesread], bytestoread);
		bytestoread -= bytesread;
	}

	if (mpq_f->mpq_b->flags & LIBMPQ_FILE_COMPRESS_MULTI) {
		libmpq_multi_decompress(buffer, &outlength, &tempbuf[0], toread);
	} else {
		memcpy(buffer, tempbuf, toread);
		outlength = toread;
	}
