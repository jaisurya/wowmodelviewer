...
#define LIBMPQ_FILE_PURE_FLAGS		6		/* file pure flags */
...
#define LIBMPQ_MPQ_ARCHIVE_VERSION	8
#define LIBMPQ_MPQ_HASHTABLE_POS	9
#define LIBMPQ_MPQ_BLOCKTABLE_POS	10
...
typedef struct {
	...
	unsigned short	formatversion;	/* 0001 for mpq_header2 */
...
typedef struct {
	unsigned long int	extblocktablepos;		/* The 0x1A51504D ('MPQ\x1A') signature */
	unsigned short	hashtableposhigh;   /* High 16 bits of the hash table offset for large archives */
	unsigned short  blocktableposhigh;  /* High 16 bits of the block table offset for large archives */
} mpq_header2;
...
typedef struct {
	...
	unsigned short	platform;	/* Platform information. */
