...
static int mpq_info_archive_info(mpq_info_options_s *mpq_info_options, unsigned char *filename) {
	...
	if (!result) {
		...
		printf("archive version:         %i\n", libmpq_archive_info(mpq_a, LIBMPQ_MPQ_ARCHIVE_VERSION));
		printf("archive size:            %i\n", libmpq_archive_info(mpq_a, LIBMPQ_MPQ_ARCHIVE_SIZE));
		printf("archive hashtable pos:   %i\n", libmpq_archive_info(mpq_a, LIBMPQ_MPQ_HASHTABLE_POS));
		printf("archive hashtable size:  %i*%i\n", libmpq_archive_info(mpq_a, LIBMPQ_MPQ_HASHTABLE_SIZE), sizeof(mpq_hash));
		printf("archive blocktable pos:  %i\n", libmpq_archive_info(mpq_a, LIBMPQ_MPQ_BLOCKTABLE_POS));
		printf("archive blocktable size: %i*%i\n", libmpq_archive_info(mpq_a, LIBMPQ_MPQ_BLOCKTABLE_SIZE), sizeof(mpq_block));
	...
	mpq_hash *mpq_h     = NULL;
	printf("hash table\n");
	printf("### name1--- name2--- idx\n");
	int i=1;
	for (mpq_h = mpq_a->hashtable; mpq_h < mpq_h_end; mpq_h++) {
	    if (mpq_h->locale != 0xFFFF)
		printf("%3d %8x %8x %3d\n", i, mpq_h->name1, mpq_h->name2, mpq_h->blockindex);
	    i++;
	}
	...
	for (mpq_b = mpq_a->blocktable; mpq_b < mpq_b_end; mpq_b++) {
	    printf("%3d %8d %8d %8d %x\n", i++, mpq_b->filepos, mpq_b->csize, mpq_b->fsize, mpq_b->flags);
	}
