...
HRESULT CAVIGenerator::InitEngineForWrite(HWND parent)
{
	...
	wxLogMessage(_T("Info: Initating AVI class object for writing."));

	// Step 0 : Let's make sure we are running on 1.1 
	DWORD wVer = HIWORD(VideoForWindowsVersion());
	if (wVer < 0x010a) {
		 // oops, we are too old, blow out of here 
		wxLogMessage(_T("Avi Error: Version of Video for Windows is too old. Come on, join the 21th century!"));
		return S_FALSE;
	}
	...
	if (hr != AVIERR_OK) {
		wxLogMessage(_T("Avi Error: AVI Engine failed to initialize. Check filename %s."), m_sFile);
		// Check it succeded.
		switch(hr) {
		case AVIERR_BADFORMAT: 
			wxLogMessage(_T("Avi Error: The file couldn't be read, indicating a corrupt file or an unrecognized format."));
			break;
		case AVIERR_MEMORY:		
			wxLogMessage(_T("Avi Error: The file could not be opened because of insufficient memory.")); 
			break;
		case AVIERR_FILEREAD:
			wxLogMessage(_T("Avi Error: A disk error occurred while reading the file.")); 
			break;
		case AVIERR_FILEOPEN:		
			wxLogMessage(_T("Avi Error: A disk error occurred while opening the file."));
			break;
		case REGDB_E_CLASSNOTREG:		
			wxLogMessage(_T("Avi Error: According to the registry, the type of file specified in AVIFileOpen does not have a handler to process it"));
			break;
		}
	...
	// Check it succeded.
	if (hr != AVIERR_OK) {
		wxLogMessage(_T("Avi Error: Stream creation failed. Check Bitmap info."));
		if (hr==AVIERR_READONLY) {
			wxLogMessage(_T("Avi Error: Read only file."));
		}
		return hr;
	}
	...
	if (hr != AVIERR_OK) {
		wxLogMessage(_T("Error: AVI Compressed Stream creation failed."));
		
		switch(hr) {
		case AVIERR_NOCOMPRESSOR:
			wxLogMessage(_T("Avi Error: A suitable compressor cannot be found."));
			break;
		case AVIERR_MEMORY:
			wxLogMessage(_T("Avi Error: There is not enough memory to complete the operation."));
			break; 
		case AVIERR_UNSUPPORTED:
			wxLogMessage(_T("Avi Error: Compression is not supported for this type of data. This error might be returned if you try to compress data that is not audio or video."));
			break;
		}
	...
	if (hr!=AVIERR_OK) {
		wxLogMessage(_T("Avi Error: Problem releasing memory"));
		return hr;
	}
	...
	if (hr != AVIERR_OK) {
		wxLogMessage(_T("Avi Error: Compressed Stream format setting failed."));
		return hr;
	}
...
{
	wxLogMessage(_T("Info: Initating AVI class object for reading."));
	...
	if (AVIStreamOpenFromFile(&m_pStream, m_sFile, streamtypeVIDEO, 0, OF_READ, NULL) !=0) {
		...
		wxLogMessage(_T("AVI Error: Failed To Open The AVI Stream."));
		ReleaseEngine();
		return;
	}
	...
	*/
	...
	if (m_pGetFrame == NULL) {
		...
		wxLogMessage(_T("AVI Error: Failed To Open The AVI Frame."));
		ReleaseEngine();
	}
}
