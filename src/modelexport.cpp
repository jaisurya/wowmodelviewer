	...
	*/
	...
	for (int i=0; i<numVerts; i++) {
		if (init == false) {
			...
			verts[i].x = m->origVertices[i].pos.x;
			verts[i].y = m->origVertices[i].pos.y;
			verts[i].z = m->origVertices[i].pos.z;
		}
	}
