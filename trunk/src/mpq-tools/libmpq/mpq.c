...
int libmpq_archive_open(mpq_archive *mpq_a, unsigned char *mpq_filename) {
	...
	while (!ncnt) {
		...
		if (mpq_a->header->id == LIBMPQ_ID_MPQ &&
		    ...
		    if ((mpq_a->header->formatversion == 0 && sizeof(mpq_header)) || (mpq_a->header->formatversion >= 1 && sizeof(mpq_header)+sizeof(mpq_header2)))
			ncnt = TRUE;
		}
...
int libmpq_archive_info(mpq_archive *mpq_a, unsigned int infotype) {
	...
	switch (infotype) {
		...
		case LIBMPQ_MPQ_UNCOMPRESSED_SIZE:
			...
			return fsize;
		case LIBMPQ_MPQ_ARCHIVE_VERSION:
			return mpq_a->header->formatversion;
		case LIBMPQ_MPQ_HASHTABLE_POS:
			return mpq_a->header->hashtablepos;
		case LIBMPQ_MPQ_BLOCKTABLE_POS:
			return mpq_a->header->blocktablepos;
		default:
			return LIBMPQ_TOOLS_SUCCESS;
	}
}
...
int libmpq_file_number(mpq_archive *mpq_a, const char *name) {
	...
	hashstart = libmpq_hash_string(mpq_a, 0, name) % mpq_a->header->hashtablesize;
	hash1 = libmpq_hash_string(mpq_a, 1, name);
	hash2 = libmpq_hash_string(mpq_a, 2, name);
...
int libmpq_file_check(mpq_archive *mpq_a, void *file, int type) {
	...
	switch (type) {
		...
		case LIBMPQ_FILE_TYPE_CHAR:
			...
			if (libmpq_file_number(mpq_a, file) != LIBMPQ_EFILE_NOT_FOUND) {
				return LIBMPQ_TOOLS_SUCCESS;
			} else {
				return LIBMPQ_EFILE_NOT_FOUND;
			}
 ...
 *  to a buffer by the given number.
 */
int libmpq_file_get(mpq_archive *mpq_a, const int number, char *dest) {
	int blockindex = -1;
	int i = 0;
	char buffer[0x1000];
	...
	if (mpq_f->mpq_b->flags & LIBMPQ_FILE_SINGLE_UNIT) {
		libmpq_file_read_single_file(mpq_a, mpq_f, mpq_f->filepos, dest, mpq_f->mpq_b->csize);
	} else {
		int pos = 0;
		while (transferred > 0) {
			...
			memcpy(&dest[pos], buffer, transferred);
			pos += transferred;
			if (transferred == 0) {
				break;
			}
...
int libmpq_file_extract(mpq_archive *mpq_a, const int number) {
	...
	if ((fsize = libmpq_file_info(mpq_a, LIBMPQ_FILE_UNCOMPRESSED_SIZE, number)) < 0)
		return fsize;

	if ((dest = malloc(fsize + 1)) == NULL) {
		return LIBMPQ_EALLOCMEM;
	}

#ifdef LINUX
	strcpy(tempfile, "Work/");
#else
	strcpy(tempfile, "Work\\");
#endif
	snprintf(tempfile+5, PATH_MAX-5, libmpq_file_name(mpq_a, number));
	mkdir(dirname(tempfile), 0755);
	...
	libmpq_file_get(mpq_a, number, dest);
	write(fd, dest, fsize);
...
/*
 *  This function tries to get the filenames for the inner block. It uses
 *  an line break listfile database.
 */
int libmpq_inner_listfile(mpq_archive *mpq_a) {
	int i = 0;
	int number, fsize;
	char *dest;

	/* extract (listfile) from mpq */
	number = libmpq_file_number(mpq_a, "(listfile)");

	if (number < 1 || number > mpq_a->header->blocktablesize) {
		return LIBMPQ_CONF_EFILE_NOT_FOUND;
	}

	/* (listfile) may not in the list */
	free(mpq_a->mpq_l->mpq_files[number-1]);
	mpq_a->mpq_l->mpq_files[number-1] = strdup("(listfile)");

	fsize = libmpq_file_info(mpq_a, LIBMPQ_FILE_UNCOMPRESSED_SIZE, number);
	if ((dest = malloc(fsize + 1)) == NULL) {
		return LIBMPQ_CONF_EFILE_NOT_FOUND;
	}
	libmpq_file_get(mpq_a, number, dest);

	/* read (listfile) and set filename */
	int pos = 0;
	char *posend;
	while(pos < fsize) {
	    posend = strchr(dest+pos, '\r');
	    posend[0] = '\0';
	    number = libmpq_file_number(mpq_a, dest+pos);
	    if (number >= 1 && number <= mpq_a->header->blocktablesize) {
	    	free(mpq_a->mpq_l->mpq_files[number-1]);
	    	mpq_a->mpq_l->mpq_files[number-1] = strdup(dest+pos);
	    }
	    pos = posend - dest + 1;
	    if (dest[pos] == '\n')
	    	pos ++;
	...
	number = libmpq_file_number(mpq_a, "(attributes)");
	if (number >= 1 && number <= mpq_a->header->blocktablesize) {
		/* filename must set by libmpq_listfile_open() before */
		free(mpq_a->mpq_l->mpq_files[number-1]);
		mpq_a->mpq_l->mpq_files[number-1] = strdup("(attributes)");
	}
